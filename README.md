...
**All figures in this section are preliminary — pre-final-run.** They were
taken on a warm database (~600k rows), not from a clean Phase-3 baseline, and
have not been written to a results file. Treat them as a starting point; every
one is reproducible by a script in this repository (commands below). CPU,
memory, and WAL figures have **not been measured yet**.
...
**Dataset.** Generated by `scripts/benchmark.mjs` — 599,600 accepted logs at
the time of the ingestion run; 599,704 rows at the time of the storage
measurement; the drain walk covered 599,635 rows.

**Batch / page sizes.** Ingestion ran at batch size 200; the drain walked
1,000-row pages.

**Methodology.** Ingestion: fixed 30-second window, 64 concurrent workers, with
a concurrent aggregate probe every second. Drain:
...
**Results (preliminary — pre-final-run):**

| Measurement | Result |
| --- | --- |
| Ingestion throughput | **19,504 logs/s** sustained over 30 s, 64 workers, batch 200 |
| Accepted / errors | 599,600 accepted, **0 errors** |
| Ingest latency p50 / p95 / p99 | 610 / 901 / 1150 ms (per whole batch of 200) |
| Aggregate p95, concurrent with ingestion | **112 ms** (requirement: < 1 s — met) |
| Drain rate | 69.6 pages/s, 69,596 rows/s |
| Page latency p50 / p95 / p99 | 11.7 / **26.1** / 79.5 ms |
| Walk integrity | 600 pages to the true end; unique rows = `COUNT(*)` exactly (599,635); 0 duplicates, 0 ordering violations |
| PostgreSQL-side page execution | 1.6 ms (`EXPLAIN (ANALYZE, BUFFERS)`: `Limit` over `Merge Append` of backward index scans, 34 shared-buffer hits, no sort node) |
| Storage | 203 MB total at 599,704 rows, of which **110 MB is indexes** — index overhead over half the table size, reported honestly |

**Target status — written down because it is missed:** the plan target of
≤ 8 ms p95 for a 1,000-row page is **currently missed at 26.1 ms**.
PostgreSQL executes the same page in 1.6 ms, so the remaining time is spent in
row materialisation, JSON serialisation, the HTTP write inside the 0.5-CPU
application, plus host→container network overhead — which is exactly where
the next optimisation phase points (candidate serialisation strategies are
sketched in `plan/01-ARCHITECTURE.md` §5).

**Bottlenecks found and optimisations applied so far:**
...
3. **Open bottleneck (page latency).** The 26.1 ms p95 versus 1.6 ms of
   database-side execution described above. This is the gap the optimisation
   phase exists to close; the honest number stands until it changes.
...
- **Page latency target not yet met (preliminary measurement).** ≤ 8 ms p95
  per 1,000-row page is the target; measured at 26.1 ms p95. See
  [Performance](#performance).
- **All performance figures are preliminary.** Taken on a warm ~600k-row
  database on a single Windows host; no clean-baseline run exists yet, and the
  >1M-row pagination gate has not been run at full scale.
