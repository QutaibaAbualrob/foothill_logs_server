...
> **You are on `perf/fastify-bun`.** This file is synced from `main`; treat
> `main` as its source and do not fork it here. This branch carries the Fastify
> HTTP layer and the Bun runtime (`Dockerfile.bun`, `docker-compose.bun.yml`,
> `bun.lock`) and is **not merged**. The compose file here is an **overlay** —
> `COMPOSE_FILE=docker-compose.yml:docker-compose.bun.yml` with its own
> `COMPOSE_PROJECT_NAME` and a non-colliding `HOST_PORT` — so a bare
> `docker compose up` in this worktree does **not** give you this stack.
>
> Note this branch is behind `main`, which has since gained migration 003, the
> validation bounds, the harness container-resolution fixes, and
> `scripts/mixed-workload.mjs`. The A/B that measured this branch was run with
> the harness from `main`'s worktree, not from here.
