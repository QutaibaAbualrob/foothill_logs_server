...
> **Superseded 2026-08-18.** `Dockerfile.bun` and `docker-compose.bun.yml` no
> longer exist: Fastify + Bun was merged to `main` and the overlay was folded
> into `Dockerfile` and `docker-compose.yml`, which now build and run Bun by
> default. The `COMPOSE_FILE=...:docker-compose.bun.yml` invocations below are
> kept as the record of how these runs were taken; they will not work against
> the current tree. See `docs/test_results/mixed-workload-baseline.md`.
...
`COMPOSE_PROJECT_NAME`. Run against any other compose project and the summary silently
